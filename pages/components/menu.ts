...
export class Menu {
    ...
    // Assertions
    constructor() {
        this.element = $('.Menu-module_firstCategories__1v_O9'); // todo el cuadro del menu
        // $ para un elemento
    ...
    public async findCategory(name : string): Promise<Category>{
        ...
        await this.categories.each(async (categoryElement) => { // por cada categoryElement dentro de categories
            const category = new Category(categoryElement, $('.SecondLevelItems_displaySubMenuDesktop__33Gpt'));
            const categoryName = await category.name.getText() // string
